...
func TestTruePositiveCalculations(t *testing.T) {
	...
	t.Run("TestAllRepositoriesOverall", func(t *testing.T) {
		// Test to calculate the overall "all" (average of all repositories)
		...
		// Method 1: Average of repository percentages
		...
		// Method 2: Total TPs / Total vulnerabilities
	...
	t.Run("TestAllEnginesGlobal", func(t *testing.T) {
		// Test to calculate the global average of each engine
		...
		// Collect percentages of each engine per repository
		for _, summary := range summaries {
			for _, eng := range engines {
	...
	t.Run("TestAllWithinRepository", func(t *testing.T) {
		// Test to calculate the "all" within a repository (average of engines)
		...
		// Calculate "all" as average of engine percentages for this repository
		...
		// Verify if the average makes sense
	...
	t.Run("TestEngineAverageComparison", func(t *testing.T) {
		// Test to compare different calculation methods per engine
		projectName := "MAD-Goat-Project/mad-deployment-service"
		engine := "IAC"

		// Method 1: Using the specific function
		...
		// Method 2: Direct manual calculation
		...
		"MAD-Goat-Project/mad-deployment-service": {
			{
				...
				Details: []handler.VulnerabilityDetail{
					...
					{
						ResultID:              "different_vulnerability_id_123", // Different ID - not a TP
						ResourceType:          "Filename",
						Resource:              "/Dockerfile",
			...
			},
			{
				...
				Details: []handler.VulnerabilityDetail{
					...
					{
						ResultID:              "b1d49eb66b250abb3c7b6b526e77f646", // Same ID - should be TP
						ResourceType:          "Package",
						Resource:              "org.apache.logging.log4j:log4j-core",
						VulnerabilityCategory: "CWE",
						VulnerabilityValue:    "74",
					...
					{
						ResultID:              "unique_vulnerable_only_id", // ID only in vulnerable - not a TP
						ResourceType:          "Package",
