...
func NewConnection(dbPath string) (*Connection, error) {
	...
	CREATE TABLE IF NOT EXISTS scans (
		...
		version TEXT NOT NULL, -- 'vulnerable' or 'expected'
        tool TEXT,
...
func (c *Connection) GetTruePositivePercentageWithCount(projectID int) (float64, int, error) {
	var tp int
	err := c.QueryRow(`
	SELECT COUNT(DISTINCT f_exp.vulnerability_id) 
	FROM findings f_exp
	JOIN scans s_exp ON f_exp.scan_id = s_exp.id
	WHERE s_exp.project_id = ? AND s_exp.version = 'expected'
        AND EXISTS (
	    ...
	    AND f_vuln.vulnerability_id = f_exp.vulnerability_id
        )
	...
	FROM findings f
	JOIN scans s ON f.scan_id = s.id
	WHERE s.project_id = ? AND s.version = 'expected'
...
func (c *Connection) GetTruePositivePercentageByEngineWithCount(engine, repo string) (float64, int, error) {
	...
	FROM findings f_exp
	JOIN scans s_exp ON f_exp.scan_id = s_exp.id
	JOIN projects p ON s_exp.project_id = p.id
	WHERE f_exp.engine = ? AND p.name = ? AND s_exp.version = 'expected'
        AND EXISTS (
            SELECT 1 FROM findings f_vuln
            JOIN scans s_vuln ON f_vuln.scan_id = s_vuln.id
	...
	JOIN scans s ON f.scan_id = s.id
	JOIN projects p ON s.project_id = p.id
	WHERE f.engine = ? AND p.name = ? AND s.version = 'expected'
