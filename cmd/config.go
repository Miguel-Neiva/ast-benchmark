...
import (
	...
	"github.com/cx-miguel-neiva/ast-benchmark/internal/handler/sonarqube"
...
func GetResults(path string, plugin string) (map[string][]handler.EngineResult, error) {
	...
	case "sonarqube":
		result, err = sonarqube.ParseReport(item)
		if err != nil {
			log.Error().Err(err).Msg("Failed to process SonarQube report")
			return nil, err
		}
